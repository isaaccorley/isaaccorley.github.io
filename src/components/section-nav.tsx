...
export function SectionNav({ sections }: SectionNavProps) {
  ...
  return (
    <>
      ...
      <nav
        className="hidden md:block pt-8 border-t border-zinc-200 dark:border-zinc-700"
        aria-label="Section navigation"
      >
        <ul className="space-y-2">
          {sections.map(({ id, label }) => (
            <li key={id}>
              <button
                onClick={() => scrollToSection(id)}
                className={`px-3 py-1.5 text-xs tracking-wider uppercase rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2 ${
                  activeSection === id
                    ? "bg-[var(--accent)] text-white font-medium"
                    : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800"
                }`}
