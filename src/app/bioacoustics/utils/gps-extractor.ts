    ...
    if (native) {
      ...
      for (const tags of Object.values(native)) {
        for (const tag of tags) {
          ...
          if (
            tag.id.toLowerCase().includes("gps") ||
...
function parseGPSValue(value: unknown): GPSCoordinates | null {
  ...
  if (typeof value === "object" && value !== null) {
    ...
    if ("latitude" in obj && "longitude" in obj) {
      const lat =
        typeof obj.latitude === "number" ? obj.latitude : parseFloat(String(obj.latitude));
      const lon =
        typeof obj.longitude === "number" ? obj.longitude : parseFloat(String(obj.longitude));
