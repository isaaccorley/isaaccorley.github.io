...
export default function BioacousticsDetectionAnalysisPage() {
  ...
  const [selectedMetric, setSelectedMetric] = useState<
    "aci" | "adi" | "ndsi" | "bi" | "combined" | "freq-bands" | "detections"
  >("combined");
  ...
  const [speciesData, setSpeciesData] = useState<
    Array<{ v5Code: string; commonName: string; thumbnail: string | null }>
  >([]);
  ...
  const setupEqualizer = useCallback((ws: WaveSurfer) => {
    ...
    const media = (
      ws as unknown as { getMediaElement?: () => HTMLMediaElement | null }
    ).getMediaElement?.();
    if (!media) {
  ...
  useEffect(() => {
    ...
    const getOrCreateTooltip = (chart: ChartJS) => {
      ...
      if (!tooltipElementRef.current) {
        ...
        tooltipEl.className = isDarkMode
          ? "bg-slate-900/95 border border-slate-700 rounded-lg p-3 shadow-xl pointer-events-none"
          : "bg-white border border-slate-300 rounded-lg p-3 shadow-xl pointer-events-none";
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            grid: {
              display: false,
              color: isDarkMode ? "rgba(148,163,184,0.1)" : "rgba(148,163,184,0.3)",
            },
            title: {
              display: true,
              text: "12s segments",
  ...
  useEffect(() => {
    ...
    let yAxisConfig: {
      ...
      beginAtZero: true,
      title: { display: true, text: "Value", color: isDarkMode ? "#cbd5e1" : "#64748b" },
    };
    ...
    if (selectedMetric === "aci") {
      ...
      yAxisConfig = {
        beginAtZero: true,
        title: { display: true, text: "ACI Value", color: isDarkMode ? "#cbd5e1" : "#64748b" },
      };
      ...
      yAxisConfig = {
        min: 0,
        max: 1,
        title: { display: true, text: "ADI (0-1)", color: isDarkMode ? "#cbd5e1" : "#64748b" },
      };
      ...
      yAxisConfig = {
        beginAtZero: true,
        title: { display: true, text: "BI Value", color: isDarkMode ? "#cbd5e1" : "#64748b" },
      };
      ...
      const aciNormalized = aciNorm.map((v) =>
        maxAci === minAci ? 0.5 : (v - minAci) / (maxAci - minAci),
      );
      ...
      const biNormalized = biNorm.map((v) =>
        maxBi === minBi ? 0.5 : (v - minBi) / (maxBi - minBi),
      );
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            ...
            ticks: { ...yAxisConfig.ticks, color: isDarkMode ? "#cbd5e1" : "#64748b" },
            title: { ...yAxisConfig.title, color: isDarkMode ? "#cbd5e1" : "#64748b" },
          },
        },
        plugins: {
          legend: {
  ...
  useEffect(() => {
    if (!audioObjectUrl || !waveContainerRef.current || !spectrogramContainerRef.current) {
      return;
    }
    ...
    if (existingWs) {
      const media = (
        existingWs as unknown as { getMediaElement?: () => HTMLMediaElement | null }
      ).getMediaElement?.();
    ...
    if (
      waveContainerRef.current?.children.length ||
      spectrogramContainerRef.current?.children.length
    ) {
    ...
    (async () => {
      ...
      console.log(
        "Creating WaveSurfer with URL:",
        currentUrl,
        "Is tracked:",
        audioBlobUrlsRef.current.has(currentUrl),
      );
      ...
      const waveSurferInstance = WaveSurfer.create({
        ...
        barWidth: 2,
        barGap: 1,
        plugins: [regionsPlugin, spectrogram, hoverPlugin],
      });
      ...
      waveSurferInstance.on("ready", () => {
        ...
        const decoded = (
          waveSurferInstance as unknown as { getDecodedData?: () => AudioBuffer | null }
        ).getDecodedData?.();
        const sr =
          decoded?.sampleRate ??
          (waveSurferInstance as unknown as { backend?: { buffer?: AudioBuffer | null } }).backend
            ?.buffer?.sampleRate ??
          null;
        ...
        if (sr && Number.isFinite(sr)) {
          ...
          const shouldRecreate =
            Math.abs(targetMax - previousFreq) > 1 || previousFreq === MAX_SPECTROGRAM_HZ;

          if (shouldRecreate) {
            console.log(
              `Sample rate detected: ${sr}Hz, recreating spectrogram with frequency range 0-${targetMax}Hz (Nyquist: ${nyquistFreq}Hz)`,
            );
            ...
            if (newSpectrogram) {
              type SpectrogramWithInit = typeof newSpectrogram & {
                init?: (ws: WaveSurfer) => void;
              };
              ...
              type WaveSurferWithRegister = WaveSurfer & {
      ...
      const hoverHandler = (time: number) => {
        console.log(
          "Hover event fired, time:",
          time,
          "predictions count:",
          clipPredictionsRef.current.length,
        );
        setCurrentTime(time);
        const predictions = clipPredictionsRef.current;
        if (predictions.length === 0) {
        ...
        const clipDuration = 12;
        const idx = Math.min(predictions.length - 1, Math.max(0, Math.floor(time / clipDuration)));
    ...
    if (eqAudioContextRef.current) {
      if (eqAudioContextRef.current.state === "suspended") {
        try {
          await eqAudioContextRef.current.resume();
          console.log(
            "AudioContext resumed in togglePlay. State:",
            eqAudioContextRef.current.state,
          );
        } catch (error) {
      ...
      if (!eqSourceNodeRef.current || !eqFiltersRef.current || eqFiltersRef.current.length === 0) {
        console.warn(
          "Equalizer not properly set up. Source:",
          !!eqSourceNodeRef.current,
          "Filters:",
          eqFiltersRef.current?.length ?? 0,
        );
      } else {
  ...
  const aggregateResults = useCallback((results: InferenceResult[]): InferenceResult => {
    ...
    const aggregated: InferenceResult["allPredictions"] = Array.from(classScores.entries()).map(
      ([classIndex, { className, humanReadableName, totalConfidence, maxConfidence, count }]) => ({
        className,
        humanReadableName,
  ...
  const processAudioFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const spectrogramResult = await audioFileToSpectrograms(file, {
          width: 1000,
          height: 257,
          sampleRate: 8000,
          duration: 12,
        ...
        setProcessingStatus(
          `Analyzing ${spectrogramResult.spectrograms.length} segment${spectrogramResult.spectrograms.length > 1 ? "s" : ""}…`,
        );
  ...
  const processAudioFromUrl = useCallback(
    async (url: string) => {
      ...
      try {
        ...
        setErrorMessage(
          error instanceof Error ? error.message : "Failed to process audio from URL",
        );
        setIsProcessing(false);
      }
    },
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const mimeLooksLikeAudio = mime.includes("audio") || mime.includes("video");
  ...
  return (
    <div
      className={`min-h-screen transition-colors ${isDarkMode ? "bg-gradient-to-b from-slate-950 via-slate-930 to-slate-950 text-slate-100" : "bg-gradient-to-b from-slate-50 via-slate-100 to-slate-50 text-slate-900"}`}
    >
      <main aria-label="AI Bioacoustics Analysis Application">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 pb-12 pt-12">
          <section
            className={`relative overflow-hidden rounded-2xl border px-6 py-6 shadow-[0_25px_80px_rgba(0,0,0,0.45)] ${isDarkMode ? "border-slate-800/70 bg-slate-900/60" : "border-slate-200 bg-white"}`}
          ...
          >
            ...
            <div
              className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(96,165,250,0.18),transparent_35%),radial-gradient(circle_at_80%_0%,rgba(52,211,153,0.18),transparent_30%),radial-gradient(circle_at_50%_90%,rgba(248,113,113,0.12),transparent_25%)]"
              aria-hidden="true"
            />
            <div className="relative flex flex-col md:flex-row gap-4 items-start md:items-center">
              <div className="flex flex-col gap-3 md:max-w-[65%]">
                <div
                  className={`inline-flex w-fit items-center gap-2 rounded-full border px-3 py-1 text-xs ${isDarkMode ? "border-slate-800/80 bg-slate-900/70 text-slate-300" : "border-slate-200 bg-slate-50 text-slate-600"}`}
                >
                ...
                <h1
                  id="page-heading"
                  className={`text-3xl font-semibold tracking-tight md:text-4xl ${isDarkMode ? "text-slate-50" : "text-slate-900"}`}
                >
                ...
                >
                  ...
                  model (Lesmeister et al., 2025) trained on 824,120 labeled spectrograms to detect
                  135 sonotypes (see gallery below). Inference is performed in the browser using the
                  onnx-runtime js library.
                </p>
          ...
          <section
            className={`rounded-2xl border p-3 shadow-lg backdrop-blur-sm ${isDarkMode ? "border-slate-800/60 bg-slate-950/80" : "border-slate-200 bg-white"}`}
          >
            <button
            ...
            >
              <h2
                className={`text-sm font-semibold uppercase tracking-[0.14em] ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}
              >
                Sonotype Gallery
              </h2>
              {isGalleryExpanded ? (
                      ...
                      .map((species) => (
                        ...
                        >
                          ...
                          <div className="text-center">
                            <p
                              className={`text-[10px] font-medium line-clamp-2 leading-tight ${isDarkMode ? "text-slate-100" : "text-slate-700"}`}
                            >
                              {species.commonName}
                            </p>
                            <p
                              className={`text-[9px] mt-0.5 ${isDarkMode ? "text-slate-500" : "text-slate-600"}`}
                            >
                              {species.v5Code}
                            </p>
                          </div>
          ...
          <div className="flex flex-col gap-8">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-stretch">
              <div
                className={`relative flex flex-col space-y-4 rounded-2xl border p-4 shadow-[0_15px_50px_rgba(0,0,0,0.4)] ${isDarkMode ? "border-slate-800/70 bg-slate-900/60" : "border-slate-200 bg-white"}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="group relative">
                      <Info
                        className={`h-4 w-4 cursor-help transition-colors ${isDarkMode ? "text-slate-500 hover:text-slate-300" : "text-slate-400 hover:text-slate-600"}`}
                      />
                      <div
                        className={`absolute left-0 top-6 z-50 hidden group-hover:block w-72 p-3 rounded-lg shadow-xl text-xs ${isDarkMode ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-slate-300 text-slate-700"} border`}
                      >
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <span id="url-help" className="sr-only">
                      Enter a URL to an audio file to analyze
                    </span>
                    <button
                      type="submit"
                      disabled={!model || isProcessing}
                ...
                <div className="flex gap-4">
                  <div
                    className={`flex-1 grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}
                  >
                    <span
                    ...
                    <div role="status" aria-live="polite" aria-atomic="true">
                      <span
                        className={`rounded-full px-2 py-1 text-xs ${isDarkMode ? "bg-slate-800/80 text-slate-200" : "bg-slate-100 text-slate-700"}`}
                      >
                        {modelStatus}
                      </span>
                    </div>
                    ...
                    >
                      {processingStatus ? (
                        isProcessing && processingProgress !== null ? (
                          <>
                            <div
                              className="h-4 w-4 animate-spin rounded-full border-2 border-slate-400 border-t-emerald-400"
                              aria-hidden="true"
                            />
                    ...
                    <span className={isDarkMode ? "text-slate-200" : "text-slate-700"}>
                      {fileName || <span className="text-slate-400">—</span>}
                    </span>
                    ...
                    <span className={isDarkMode ? "text-slate-200" : "text-slate-700"}>
                      {audioMetadata?.location?.lat !== undefined &&
                      audioMetadata.location?.lon !== undefined ? (
              ...
              <section
                className={`w-full flex flex-col justify-self-end space-y-4 rounded-2xl border p-4 shadow-[0_15px_50px_rgba(0,0,0,0.4)] ${isDarkMode ? "border-slate-800/70 bg-slate-900/60" : "border-slate-200 bg-white"}`}
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="group relative">
                      ...
                      >
                        ...
                        <p className="mt-2">
                          An equalizer (EQ) allows you to adjust the volume of specific frequency
                          ranges. Use it to:
                        </p>
                        <ul className="space-y-1 mt-2">
                          <li>
                            • <strong>Boost frequencies</strong> where bird calls occur (1-8 kHz)
                          </li>
                          <li>
                            • <strong>Reduce low frequencies</strong> to minimize background noise
                            (wind, traffic)
                          </li>
                          <li>
                            • <strong>Enhance clarity</strong> of specific vocalizations by
                            adjusting individual bands
                          </li>
                        </ul>
                      </div>
                    </div>
                  ...
                  <span
                    ...
                    aria-label="Equalizer range"
                  >
                    -40 dB to +40 dB
                  </span>
                </div>
                <div
                  className={`flex flex-col gap-2 rounded-lg border p-3 ${isDarkMode ? "border-slate-800/70 bg-slate-950/70" : "border-slate-200 bg-slate-50"}`}
                  role="group"
                  aria-label="Equalizer frequency controls"
                >
                  ...
                  <div className="hidden md:flex items-end gap-3 overflow-x-auto pb-1">
                    {EQ_BANDS.map((band, idx) => (
                      <div
                        key={band}
                        className={`flex flex-col items-center gap-1 text-xs ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}
                      >
                        <div className="relative flex h-48 w-8 items-center justify-center">
                          <div
                            className="absolute inset-y-2 left-1/2 w-[2px] -translate-x-1/2 rounded-full bg-slate-700/70"
                            aria-hidden="true"
                          />
                          <input
                            type="range"
                        ...
                        <span
                          aria-label={`${band >= 1000 ? `${band / 1000} kilohertz` : `${band} hertz`}`}
                        >
                  ...
                  <div className="flex md:hidden flex-col gap-2">
                    {EQ_BANDS.filter((band) => band <= 4000).map((band, idx) => (
                      <div key={band} className="flex items-center gap-2">
                        <span
                          className="w-12 text-xs text-slate-300"
                          aria-label={`${band >= 1000 ? `${band / 1000} kilohertz` : `${band} hertz`}`}
                        >
                          {band >= 1000 ? `${band / 1000}k` : band}
                        </span>
                        <input
                          ...
                          aria-valuetext={`${eqGains[idx] > 0 ? "+" : ""}${eqGains[idx].toFixed(1)} dB`}
                        />
                        <span className="w-10 text-[10px] text-slate-300 text-right">
                          {eqGains[idx] > 0 ? "+" : ""}
                          {eqGains[idx].toFixed(0)}
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            <section aria-labelledby="waveform-heading" className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div className="group relative">
                    ...
                    <div
                      className={`absolute left-0 top-6 z-50 hidden group-hover:block w-80 p-3 rounded-lg shadow-xl text-xs ${isDarkMode ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-slate-300 text-slate-700"} border`}
                    >
                      <strong className={isDarkMode ? "text-slate-200" : "text-slate-900"}>
                        Visualization Guide:
                      </strong>
                      <ul className="space-y-1 mt-2">
                        <li>
                          • <strong>Top:</strong> Time-series waveform showing audio amplitude over
                          time
                        </li>
                        <li>
                          • <strong>Bottom:</strong> Spectrogram displaying frequency content
                          (darker = less energy, brighter = more energy)
                        </li>
                        <li>
                          • <strong>Bird frequencies:</strong> Most bird calls/songs occur between
                          1-8 kHz
                        </li>
                        <li>
                          • <strong>Hover:</strong> Move your cursor over the waveform to see the
                          top detected species at that time point
                        </li>
                      </ul>
                    </div>
                  </div>
                ...
                <div
                  className={`flex items-center gap-3 text-xs ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}
                  role="group"
                ...
                >
                  <span aria-live="off" aria-atomic="true">
                    ...
                    <span className="sr-only"> of </span> / {formatTime(audioDuration)}
                  </span>
                  <button
                    type="button"
                    onClick={togglePlay}
                  ...
                  <div className="hidden md:flex items-center gap-2">
                    ...
                    />
                    <span className="text-[10px] text-slate-400 w-8" aria-hidden="true">
              ...
              >
                <div ref={waveContainerRef} className="absolute inset-0" suppressHydrationWarning />
                {!audioObjectUrl && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                    <p className={`text-sm ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>
                      Upload or paste a URL to see the waveform
                    </p>
                  </div>
                )}
              ...
              >
                ...
                {!audioObjectUrl && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                    <p className={`text-sm ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>
                      Upload or paste a URL to see the spectrogram
                    </p>
                  </div>
                )}
                {audioObjectUrl && isProcessing && (
              ...
              >
                <div
                  className={`flex items-center justify-between text-[11px] ${isDarkMode ? "text-slate-300" : "text-slate-600"}`}
                >
                  <span>
                    {hoverInfo ? `Segment @ ${formatTime(hoverInfo.time)}` : "Hover over waveform"}
                  </span>
                  <span>Top 3</span>
                </div>
                    ...
                    hoverInfo.items.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`flex-shrink-0 ${getConfidenceColorClass(item.confidence)}`}
                          >
            ...
            <section
              className={`space-y-4 rounded-2xl border p-6 shadow-[0_15px_50px_rgba(0,0,0,0.4)] ${isDarkMode ? "border-slate-800/70 bg-slate-900/60" : "border-slate-200 bg-white"}`}
              aria-labelledby="results-heading"
              aria-busy={isProcessing && !classificationResult ? true : undefined}
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="group relative">
                  ...
                  <div
                    className={`absolute left-0 top-6 z-50 hidden group-hover:block w-72 p-3 rounded-lg shadow-xl text-xs ${isDarkMode ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-slate-300 text-slate-700"} border`}
                  >
                    Maximum detection confidence for each 12-second audio segment. Shows which
                    species were detected with highest probability across the recording timeline.
                  </div>
                </div>
              </div>
              ...
              {clipPredictions.length > 0 && (
                <div
                  className={`rounded-lg border p-3 ${isDarkMode ? "border-slate-800/70 bg-slate-950/60" : "border-slate-200 bg-white"}`}
                >
                  <div className="relative h-32">
                    <canvas
                      ...
                      role="img"
                      aria-label="Bar chart showing detection confidence for each 12-second segment"
                    />
                  </div>
                </div>
              )}
              ...
              {isProcessing && !classificationResult ? (
                <div className="space-y-1.5" role="status" aria-label="Loading detection results">
                  ...
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div
                      key={i}
                      className={`flex flex-col gap-1 rounded-xl border px-3 py-1.5 animate-pulse ${isDarkMode ? "border-slate-800/80 bg-slate-950/60 shadow-inner shadow-black/30" : "border-slate-200 bg-slate-50 shadow-sm"}`}
                      aria-hidden="true"
                    >
                      <div className="flex items-center justify-between">
                ...
                <div className="space-y-1.5">
                  <div
                    ...
                    role="list"
                    aria-label="Detected species ranked by confidence"
                  >
                    <div
                      className={`flex items-center justify-between text-[11px] uppercase tracking-[0.12em] ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}
                      aria-hidden="true"
                    >
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              {pred.humanReadableName &&
                                pred.humanReadableName !== pred.className && (
                                  <span
                                    className={`text-xs font-mono px-1.5 py-0.5 rounded border ${isDarkMode ? "text-slate-500 bg-slate-800/50 border-slate-700/50" : "text-slate-700 bg-white border-slate-300"}`}
                                  >
                                    {pred.className}
                                  </span>
                                )}
                              <div className="flex items-center gap-2">
                                {(() => {
                                  ...
                                  const displayName = (pred.humanReadableName ?? pred.className)
                                    .replace(/\s*\([^)]*\)\s*/g, "")
                                    .trim();
                                  const isBird =
                                    !nonBirdClasses.includes(displayName) &&
                            ...
                            <span
                              className={`w-14 text-right text-sm font-semibold ${getConfidenceColorClass(pred.maxConfidence ?? pred.confidence)}`}
                            >
                ...
                <div className="space-y-3">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div
                      key={i}
                      className={`flex flex-col gap-1 rounded-xl border px-3 py-2 animate-pulse ${isDarkMode ? "border-slate-800/80 bg-slate-950/60 shadow-inner shadow-black/30" : "border-slate-200 bg-slate-50 shadow-sm"}`}
                      aria-hidden="true"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div
                            className={`h-10 w-10 rounded-lg ${isDarkMode ? "bg-slate-700/50" : "bg-slate-200"}`}
                          />
                          <div
                            className={`h-4 w-32 rounded ${isDarkMode ? "bg-slate-700/50" : "bg-slate-200"}`}
                          />
                        </div>
                        <div
                          className={`h-4 w-12 rounded ${isDarkMode ? "bg-slate-700/50" : "bg-slate-200"}`}
                        />
                      </div>
                    </div>
                  ))}
            ...
            <section
              className={`space-y-4 rounded-2xl border p-6 shadow-[0_15px_50px_rgba(0,0,0,0.4)] ${isDarkMode ? "border-slate-800/70 bg-slate-900/60" : "border-slate-200 bg-white"}`}
              aria-labelledby="acoustic-metrics-heading"
            >
              <button
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="group relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    >
                      ...
                      Due to 8 kHz sample rate, frequency analysis is capped at 4 kHz (Nyquist
                      frequency). Full biophony range (2-8 kHz) requires higher sample rates.
                    </div>
                  </div>
                </div>
                {isTemporalAnalysisExpanded ? (
              ...
              {isTemporalAnalysisExpanded && (
                <div className="space-y-4">
                  ...
                  >
                    Species detections, ecological metrics, and frequency band analysis over time.
                    Select a view to explore detection patterns and habitat quality indicators.
                  </p>

                  {isProcessing &&
                  acousticIndicesData.length === 0 &&
                  clipPredictions.length === 0 ? (
                    <>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                          <div
                            key={i}
                            className={`h-8 w-32 rounded-lg animate-pulse ${isDarkMode ? "bg-slate-800/50" : "bg-slate-200"}`}
                          />
                        ))}
                      </div>

                      <div
                        className={`rounded-lg border p-3 h-16 animate-pulse ${isDarkMode ? "border-slate-800/70 bg-slate-950/40" : "border-slate-200 bg-slate-50"}`}
                      />

                      <div
                        className={`rounded-xl border p-4 ${isDarkMode ? "border-slate-800/70 bg-slate-950/60" : "border-slate-200 bg-white shadow-sm"}`}
                      >
                        <div
                          className={`h-64 w-full rounded animate-pulse flex items-center justify-center ${isDarkMode ? "bg-slate-800/50" : "bg-slate-100"}`}
                        >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[1, 2, 3, 4].map((i) => (
                          <div
                            key={i}
                            className={`rounded-lg border p-3 space-y-2 ${isDarkMode ? "border-slate-800/70 bg-slate-950/60" : "border-slate-200 bg-white shadow-sm"}`}
                          >
                            <div
                              className={`h-3 w-16 rounded animate-pulse ${isDarkMode ? "bg-slate-800/50" : "bg-slate-200"}`}
                            />
                            <div
                              className={`h-6 w-20 rounded animate-pulse ${isDarkMode ? "bg-slate-800/50" : "bg-slate-200"}`}
                            />
                          </div>
                        ))}
                      </div>
                    ...
                    <>
                      ...
                      {/* Metric Description */}
                      <div
                        className={`rounded-lg border p-3 text-xs ${isDarkMode ? "border-slate-800/70 bg-slate-950/40 text-slate-300" : "border-slate-200 bg-white text-slate-700 shadow-sm"}`}
                      >
                        {selectedMetric === "combined" && (
                          <>
                            <strong>Acoustic Indices Combined:</strong> Normalized view of all four
                            acoustic indices for easy comparison. Values are scaled to 0-1 range.
                            Look for patterns across multiple metrics to assess ecosystem health.
                          </>
                        )}
                        {selectedMetric === "freq-bands" && (
                          <>
                            <strong>Frequency Band Analysis:</strong> Energy distribution across
                            ecological ranges.
                        ...
                        {selectedMetric === "aci" && (
                          <>
                            <strong>ACI (Acoustic Complexity Index):</strong> Measures sound
                            intensity variability. Higher values indicate more complex soundscapes
                            with bird activity.
                          </>
                        )}
                        {selectedMetric === "adi" && (
                          <>
                            <strong>ADI (Acoustic Diversity Index):</strong> Shannon entropy across
                            frequency bins (0-1). Higher values indicate even distribution of sound
                            energy, suggesting biodiverse communities.
                          </>
                        )}
                        {selectedMetric === "ndsi" && (
                          <>
                            <strong>NDSI (Normalized Difference Soundscape Index):</strong> Ratio of
                            biological to human sounds (-1 to +1). Positive values indicate natural
                            soundscapes; negative values indicate human noise pollution.
                          </>
                        )}
                        {selectedMetric === "bi" && (
                          <>
                            <strong>BI (Bioacoustic Index):</strong> Total sound energy in bird
                            frequency range (2-4 kHz). Higher values suggest more bird activity and
                            abundance.
                          </>
                        )}
                      </div>
                      ...
                      {/* Chart */}
                      <div
                        className={`rounded-xl border p-4 ${isDarkMode ? "border-slate-800/70 bg-slate-950/60" : "border-slate-200 bg-white"}`}
                      >
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {selectedMetric === "aci" && acousticIndicesData.length > 0 && (
                          <>
                            <div
                              className={`rounded-lg border p-3 ${isDarkMode ? "border-slate-800/70 bg-slate-950/60" : "border-slate-200 bg-white shadow-sm"}`}
                            >
                              <div
                                className={`text-[10px] uppercase tracking-wider ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}
                              >
                                Mean ACI
                              </div>
                              <div className="text-lg font-semibold text-blue-300">
                                {(
                                  acousticIndicesData.reduce((sum, d) => sum + d.aci, 0) /
                                  acousticIndicesData.length
                                ).toFixed(2)}
                              </div>
                            </div>
                            <div
                          ...
                          <>
                            ...
                            >
                              ...
                              <div className="text-lg font-semibold text-emerald-300">
                                {(
                                  acousticIndicesData.reduce((sum, d) => sum + d.adi, 0) /
                                  acousticIndicesData.length
                                ).toFixed(3)}
                              </div>
                            </div>
                          ...
                          <>
                            ...
                            >
                              ...
                              <div className="text-lg font-semibold text-orange-300">
                                {(
                                  acousticIndicesData.reduce((sum, d) => sum + d.ndsi, 0) /
                                  acousticIndicesData.length
                                ).toFixed(3)}
                              </div>
                            </div>
                          ...
                          <>
                            ...
                            >
                              ...
                              <div className="text-lg font-semibold text-purple-300">
                                {(
                                  acousticIndicesData.reduce((sum, d) => sum + d.bi, 0) /
                                  acousticIndicesData.length
                                ).toFixed(3)}
                              </div>
                            </div>
                    ...
                    <>
                      ...
                      <div
                        className={`rounded-xl border p-8 ${isDarkMode ? "border-slate-800/70 bg-slate-950/60" : "border-slate-200 bg-white shadow-sm"}`}
                      >
                        <div className="flex flex-col items-center justify-center gap-3 text-center">
                          <div
                            className={`rounded-full p-4 ${isDarkMode ? "bg-slate-800/50" : "bg-slate-100"}`}
                          >
                            <svg
                              className={`h-8 w-8 ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
                          ...
                          <div className="space-y-1">
                            ...
                            <p
                              className={`text-xs max-w-md ${isDarkMode ? "text-slate-500" : "text-slate-600"}`}
                            >
                              Upload an audio file to see species detections, acoustic indices, and
