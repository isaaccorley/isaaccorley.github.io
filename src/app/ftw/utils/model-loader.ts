...
async function fetchZipWithCache(url: string): Promise<ArrayBuffer> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch model archive: ${response.status} ${response.statusText}`);
  }
    ...
    await Promise.all(
      Object.values(zip.files)
        ...
        .map(async (file) => {
          ...
          const type = lower.endsWith(".json") ? "application/json" : "application/octet-stream";
...
export async function loadModelFromOption(
  option: ModelOption,
): Promise<tf.GraphModel | InferenceSession> {
...
async function loadOnnxModelFromOption(option: ModelOption): Promise<InferenceSession> {
  try {
    ...
    if (typeof error === "number") {
      ...
      throw new Error(
        `${errorMessage}. This usually indicates a problem with the model file format or compatibility.`,
      );
    }
